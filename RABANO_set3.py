...
def tic_tac_toe(board):
    ...
    for row in range(0, size-1+1):
        symbol = board[row][0]
        if board[row] == [symbol]*size and symbol != '':
            return symbol

        cross_one.append(board[row][row])
        cross_two.append(board[row][size-(row+1)])

    for position in range(0, size-1+1):
        for row in range(0, size-1+1):
            vertical.append(board[row][position])

        symbol = board[0][position]
        if vertical == [symbol]*size and symbol != '':
            return symbol
        vertical = []

    symbol = board[0][0]
    if cross_one == [symbol]*size and symbol != '':
        return symbol

    symbol = board[0][size-1]
    if cross_two == [symbol]*size and symbol != '':
        return symbol
